...
- Use `scripts/start-temporal-ir-training-container.ps1` for current Qwen3.5 CUDA/Unsloth training jobs. Example start: `powershell -ExecutionPolicy Bypass -File scripts\start-temporal-ir-training-container.ps1 -AdapterName "qwen-temporal-ir-qwen35-08b-bf16-chat-next-lora" -PromptFormat chat -NoLoadIn4Bit`.
- Use the same container script for status/log tailing. Example status: `powershell -ExecutionPolicy Bypass -File scripts\start-temporal-ir-training-container.ps1 -AdapterName "qwen-temporal-ir-qwen35-08b-bf16-chat-next-lora" -Status -Tail 40`.
- `scripts/start-temporal-ir-training.ps1` is the legacy WSL venv lane. It now resolves the current repo root into WSL instead of assuming a D-drive checkout; do not hardcode `D:\bench` paths in new training or staging commands.
- The async launchers write `.log`, `.pid`, or container logs under ignored `api/reports/temporal-ml/`. Treat those as local experiment artifacts, not durable source. If a one-off manual WSL `nohup` command is unavoidable, avoid unescaped PowerShell `$!` / `$(...)` expansion; prefer the launcher script instead.
