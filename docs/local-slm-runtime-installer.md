...
- That promotion run predates the later Plan-IR safety validators. The latest exact validator head, commit `96e4b51`, passed `202/202` required routed cases, `163/163` required model-owned cases, and `28/41` diagnostic cases with boundary p95 `3009ms`. Its separate production-routing smoke passed `51/51` with 37 model calls and a `3879ms` maximum. The validator now fails closed on unscoped copied-prose clocks and relative-day words, recognizes pronoun-linked and third-person range endpoints, and treats any epoch-bearing non-resolved diagnostic as unsafe. Earlier correctness-clean samples ranged from p95 `2665ms` to `6802ms`, so endpoint variance remains visible rather than being treated as resolved.
