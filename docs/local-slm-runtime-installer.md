...
- That promotion run predates the later Plan-IR safety validators. The latest exact-head boundary, including ordered endpoint binding, reinforced range/shift phrasings, clock-ownership guards, and legacy clarification grounding, passed `201/201` required routed cases and `162/162` required model-owned cases with routed median `1251ms` and p95 `3397ms`. An earlier consecutive sample was correctness-clean but missed the latency gate at p95 `6802ms`, so endpoint variance remains visible rather than being treated as resolved.
