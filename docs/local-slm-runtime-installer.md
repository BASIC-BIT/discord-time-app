...
- That promotion run predates the later Plan-IR safety validators. The latest exact validator head, commit `072ebfc`, passed `202/202` required routed cases, `163/163` required model-owned cases, and `28/41` diagnostic cases with boundary p95 `2887ms`; the subsequent commit only updates this evidence. Earlier correctness-clean samples ranged from p95 `2665ms` to `6802ms`, so endpoint variance remains visible rather than being treated as resolved.
