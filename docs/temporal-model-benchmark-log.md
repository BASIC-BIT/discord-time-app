...
- Current generated expanded dataset: `2580` rows with splits `2046/270/264` after adding ordinal-weekday explicit-month, bare-minute ambiguity, fuzzy-month suffix, and bounded noisy-human-input rows for the next retrain. The current promoted adapter was trained on the prior `2564`-row month-clock dataset.
